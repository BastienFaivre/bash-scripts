#!/bin/bash
#
# Manage snap packages and update the package list
...
usage() {
  echo "Usage: snap-manage [COMMAND] <package> [OPTIONS]"
  echo 'COMMAND:'
...
update_package_list() {
  ...
  line=$(grep -xn "${package}" \
    "${PACKAGE_LISTS_REPO_PATH}"/lists/snap-packages.txt | cut -d: -f1)
  # start trap here since grep returns error if the package is not found
  trap 'return 1' ERR
  if [[ "${install}" = 'true' ]]; then
    # if the package is not in the list, add it
    if [[ -z "${line}" ]]; then
      echo "${package}" >> "${PACKAGE_LISTS_REPO_PATH}"/lists/snap-packages.txt
      # update git repository
      git -C "${PACKAGE_LISTS_REPO_PATH}" add ./lists/snap-packages.txt
      git -C "${PACKAGE_LISTS_REPO_PATH}" commit -m "feat: Added snap package ${package}"
      git -C "${PACKAGE_LISTS_REPO_PATH}" push
    fi
  else
    # if the package is in the list, remove it
    if [[ ! -z "${line}" ]]; then
      sed -i "${line}d" "${PACKAGE_LISTS_REPO_PATH}"/lists/snap-packages.txt
      # update git repository
      git -C "${PACKAGE_LISTS_REPO_PATH}" add ./lists/snap-packages.txt
      git -C "${PACKAGE_LISTS_REPO_PATH}" commit -m "feat: Removed snap package ${package}"
      git -C "${PACKAGE_LISTS_REPO_PATH}" push
    fi
  fi
  trap - ERR
...
snap info "${package}" > /dev/null 2>&1
if [[ "$?" -ne 0 ]]; then
